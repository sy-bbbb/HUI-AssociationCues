...
#if UNITY_RENDER_PIPELINES_UNIVERSAL_17
    ...
    internal static class PlatformValidation
    {
        ...
        private static void InitializePlatformValidation()
        {
            ...
            BuildValidationRule[] wsaValidationRules = new BuildValidationRule[] { GenerateHL2RealtimeGIRule(), GenerateHL2QualityRule(),
                GenerateHL2RenderAndDepthSubmissionModeRule(), GenerateHL2DX11Rule(), GenerateHL2RenderGraphRule() };
        ...
        private static BuildValidationRule GenerateHL2DX11Rule()
        {
            return new BuildValidationRule()
            {
                IsRuleEnabled = IsHL2RulesetEnabled,
                Category = "Mixed Reality OpenXR - HoloLens 2 Ruleset",
                Message = "[Optional] Switch to DX11 Graphics API if building apps for Hololens 2 to obtain the best performance.",
                CheckPredicate = () =>
                {
                    GraphicsDeviceType[] gfxTypes = PlayerSettings.GetGraphicsAPIs(BuildTarget.WSAPlayer);
                    return (gfxTypes.Length >= 1 && gfxTypes[0] == GraphicsDeviceType.Direct3D11);
                },
                FixItMessage = "Set Graphics API to D3D11 in the Player Setting under Universal Windows Platform Setting.",
                FixIt = () => PlayerSettings.SetGraphicsAPIs(BuildTarget.WSAPlayer, new[] { GraphicsDeviceType.Direct3D11 }),
                Error = false,
                HelpText = PerformanceHelpLinkText + "\r\n\r\n" + AboutValidationRulesetsHelpText,
                HelpLink = PerformanceHelpLink
            };
        ...
        private static BuildValidationRule GenerateHL2RenderGraphRule()
        {
            return new BuildValidationRule()
            {
                IsRuleEnabled = IsHL2RulesetEnabled,
                Category = "Mixed Reality OpenXR - HoloLens 2 Ruleset",
                Message = "[Optional] Disable URP Render Graph if build apps for Hololens 2 to obtain the best performance.",
                CheckPredicate = () => 
                {
#if UNITY_RENDER_PIPELINES_UNIVERSAL_17
...
#else
                    return true;
#endif //UNITY_RENDER_PIPELINES_UNIVERSAL_17
                },
                FixIt = () => 
                {
#if UNITY_RENDER_PIPELINES_UNIVERSAL_17
                ...
                FixItMessage = "[Optional] Disable URP Render Graph/Enable Render Graph Compatibility Mode for Hololens 2 App to obtain the best performance.",
                Error = false,
                HelpText = PerformanceHelpLinkText + "\r\n\r\n" + AboutValidationRulesetsHelpText,
                HelpLink = PerformanceHelpLink
            };
